...
export default function EditarEvento() {
    ...
    async function fetchEvento() {
      ...
      if (data) {
        setForm({
          ...
          promo_code: data.promo_code || 'NOS4',
  ...
  if (fetching) return (
    <div style={{ minHeight: '100vh', background: '#080808', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
      Carregando...
    </div>
  )
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: '#080808' }}>
        ...
        <main className="main-content">
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
            ...
            <h1 style={{ fontSize: '1.8rem', fontWeight: 'bold' }}>Editar Evento</h1>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ maxWidth: '800px', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={fld}>
              ...
              <input style={inp} name="location" value={form.location} onChange={handleChange} />
            </div>

            <div style={fld}>
              <label style={lbl}>Descrição</label>
              <textarea style={{ ...inp, minHeight: '100px', resize: 'vertical' }} name="description" value={form.description} onChange={handleChange} />
            </div>

            <div className="grid-2">
              <div style={fld}>
                <label style={lbl}>Link de ingresso</label>
                <input style={inp} name="ticket_link" value={form.ticket_link} onChange={handleChange} />
              </div>
              <div style={fld}>
                <label style={lbl}>Link Lista VIP</label>
                <input style={inp} name="vip_link" value={form.vip_link} onChange={handleChange} />
              </div>
            </div>
